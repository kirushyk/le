...
le_sequential_class_init (LeSequentialClass *klass)
{
  ...
  LE_MODEL_CLASS (klass)->get_gradients =
      (GList * (*)(LeModel *, const LeTensor *, const LeTensor *)) le_sequential_get_gradients;
}
...
LeTensor *le_sequential_predict (LeSequential *self, const LeTensor *x);

GList *le_sequential_get_gradients (LeSequential *self, const LeTensor *x, const LeTensor *y);
...
forward_propagation (LeSequential *self, const LeTensor *x, GList **inputs)
{
  ...
  for (GList *current = priv->layers; current != NULL; current = current->next) {
...
le_sequential_get_gradients (LeSequential *self, const LeTensor *x, const LeTensor *y)
{
  ...
  // LE_INFO("Output stats:\n\tmin: %f\n\tmax: %f\n\tmean: %f\n\tdeviation: %f", signal_stats.min, signal_stats.max,
  // signal_stats.mean, signal_stats.deviation);
  ...
  if (current_layer_iterator && current_layer_iterator->data) {
    ...
    activation_loss_backward =
        activation_loss_backward_fn (le_activation_layer_get_activation (last_layer), priv->loss);
  }
  if (last_layer && activation_loss_backward) {
    ...
    // LE_INFO("Loss derivative stats:\n\tmin: %f\n\tmax: %f\n\tmean: %f\n\tdeviation: %f", signal_stats.min,
    // signal_stats.max, signal_stats.mean, signal_stats.deviation);
  }
  ...
  for (/* current = g_list_last(self->layers), inputs = g_list_last(inputs) */;
    ...
    LeTensor *input_gradient =
        le_layer_backward_prop (current_layer, cached_input, cached_output, signal, &current_layer_param_gradients);
    le_tensor_unref (signal);
    ...
    // signal_stats.mean, signal_stats.deviation);
    for (GList *current_gradient = current_layer_param_gradients; current_gradient;
...
le_sequential_estimate_gradients (LeSequential *self, const LeTensor *x, const LeTensor *y, gfloat epsilon)
{
  ...
  for (GList *params_iterator = le_model_get_parameters (LE_MODEL (self)); params_iterator;
...
le_sequential_check_gradients (LeSequential *self, const LeTensor *x, const LeTensor *y, gfloat epsilon)
{
  ...
  for (gradients_iterator = gradients, gradients_estimations_iterator = gradients_estimations;
       gradients_iterator && gradients_estimations_iterator; gradients_iterator = gradients_iterator->next,
      gradients_estimations_iterator = gradients_estimations_iterator->next) {
...
le_sequential_to_dot (LeSequential *self, const char *filename)
{
  ...
  for (GList *current = priv->layers; current != NULL; current = current->next) {
    ...
    fprintf (fout, "%s [shape=record label=\"{%s|%s|%d Parameters}\"];\n", le_layer_get_name (current_layer),
        le_layer_get_name (current_layer), le_layer_get_description (current_layer),
